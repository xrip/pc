...
void
operator_advance_drums(op_type *op_pt1, int32_t vib1, op_type *op_pt2, int32_t vib2, op_type *op_pt3, int32_t vib3) {
...
void change_attackrate(Bitu regbase, op_type *op_pt) {
    ...
    if (attackrate) {
        fltype f = (fltype) (pow(FL2, (fltype) attackrate + (op_pt->toff >> 2) - 1) * attackconst[op_pt->toff & 3] *
                             recipsamp);
...
void change_frequency(Bitu chanbase, Bitu regbase, op_type *op_pt) {
    // frequency
    uint32_t frn =
            ((((uint32_t) adlibreg[ARC_KON_BNUM + chanbase]) & 3) << 8) + (uint32_t) adlibreg[ARC_FREQ_NUM + chanbase];
    // block number/octave
...
void adlib_init(uint32_t samplerate) {
    ...
    for (i = 0; i < TREMTAB_SIZE; i++) {
        ...
        fltype trem_val2 = (fltype) ((fltype) ((int32_t) (trem_table_int[i] / 4)) * 1.2 / 6.0 /
...
inline static void clipit16(int32_t ival, int16_t *outval) {
    if (ival < 32768) {
        if (ival > -32769) {
            *outval = (int16_t) ival;
        } else {
