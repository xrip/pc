...
void blaster_putreadbuf(uint8_t value) {
    if (readlen == 16) return;

    readbuf[readlen++] = value;
}

uint8_t blaster_getreadbuf() {
    uint8_t ret;
...
void blaster_writecmd(uint8_t value) {
    switch (lastcmd) {
        ...
        case 0x40: //set time constant
            ...
            lastcmd = 0;
#ifdef DEBUG_BLASTER
            printf("[BLASTER] Set time constant: %u (Sample rate: %f Hz)\r\n", value, sb_samplerate);
#endif
            return;
