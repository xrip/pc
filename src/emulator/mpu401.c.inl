...
static void parse_midi(uint32_t midi_command) {
    struct {
        uint8_t command;
        uint8_t note;
        uint8_t velocity;
...
int16_t midi_sample() {
    ...
    for (int channel = 0; channel < 16; channel++) {
        if (midi_channels[channel].playing) {
            float amplitude = (float)midi_channels[channel].velocity / 127.0f;
            float frequency = note_frequencies[midi_channels[channel].note];

            sample += amplitude * sinf(6.28 * frequency * (++midi_channels[channel].sample_position / SOUND_FREQUENCY));
        }
    }
    return (int16_t)(sample * 2047);
}