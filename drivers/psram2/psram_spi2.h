...
#define PIN_CS   18 // Chip Select (SPI0 CS )
#define PIN_SCK  19 // Clock       (SPI0 SCK)
#define PIN_MOSI 20 // Output      (SPI0 TX )
#define PIN_MISO 21 // Input       (SPI0 RX )
#endif
...
//    printf("about to init fudge\n", spi_sm);
  ...
  //  printf("Inited SPI PIO... at sm %d pio %d\n", spi.sm, spi.pio);
    ...
    pio_spi_write_read_blocking(&spi, &psram_reset_en_cmd, 1, NULL, 0);
    busy_wait_us(50);
    uint8_t psram_reset_cmd = 0x99u;
    pio_spi_write_read_blocking(&spi, &psram_reset_cmd, 1, NULL, 0);
    busy_wait_us(100);
...
__force_inline void psram_write8(pio_spi_inst_t* spi, uint32_t addr, uint8_t val) {
    ...
    pio_spi_write_read_blocking(spi, command, sizeof(command), NULL, 0);
};
